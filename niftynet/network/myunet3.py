...
class MyUNet3D3(TrainableLayer):
    """
    ...
    def __init__(self,
                 ...
                 acti_func='leakyrelu',
                 name='NoNewNet'):
        super(MyUNet3D3, self).__init__(name=name)

        self.n_features = [16, 32, 64]
        self.n_features_down = [4,16,32]
    ...
    def layer_op(self, thru_tensor, is_training=True, **unused_kwargs):
        ...
        # crop 128x128x128 from original 200X200X200 to start
        crop_op = CropLayer(border=36, name='cropping_input')
        ...
        block_layer = PeelBlock(tuple(self.n_features_down[2] for i in range(4)),
                                tuple(3 for i in range(4)),
                                w_initializer=self.initializers['w'],
                                w_regularizer=self.regularizers['w'],
                                acti_func=self.acti_func,
                                name='L4')
        thru_tensor = block_layer(thru_tensor, is_training)
        # crop 32x32x32 from 64x64x64
        crop_op = CropLayer(border=16, name='cropping_input')
        thru_tensor = crop_op(thru_tensor)
