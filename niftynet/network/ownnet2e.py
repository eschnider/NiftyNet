...
class Ownnet2e(BaseNet):
    ...
    def __init__(self,
        ...
        self.layers = [
            ...
            {'name': 'linear_upsampling', 'n_features': 32, 'kernel_size': 3},
    ...
    def layer_op(self, images, is_training, layer_id=-1, **unused_kwargs):
        ...
        ### very first common down convolution layer
        ...
        first_conv_layer = ConvolutionalLayer(
            ...
            name=params['name'])
        first_down = first_conv_layer(images, is_training)
        layer_instances.append((first_conv_layer, first_down))

        ### giant path dilated convolution blocks, 4 times dilated by 4, 4 times by 8, 1 time by 16 (from 256 -> 128)

        params = self.giant_layers[0]
        with DilatedTensor(first_down, dilation_factor=4) as dilated:
        ...
        params = self.giant_layers[2]
        with DilatedTensor(flow, dilation_factor=16) as dilated:
            for j in range(params['repeat']):
                res_block = HighResBlock(
                    params['n_features'],
                    params['kernels'],
        ...
        # upsampling giant flow (128->256)
        up_shape = [2 * int(flow.shape[i]) for i in (1, 2, 3)]
        upsample_op = LinearResizeLayer(up_shape)
        flow = upsample_op(flow)
        layer_instances.append((upsample_op, flow))

        # crop 256x256x256 from original 512x512x512 to start
        crop_op = CropLayer(border=128, name='cropping_input')
        full_res_256 = crop_op(images)
        ...
        full_res_flow = crop_op(full_res_256)
        layer_instances.append((crop_op, full_res_flow))


        ### 256 -> 128 common down convolution layer
        params = self.layers[0]
        first_conv_layer = ConvolutionalLayer(
            n_output_chns=params['n_features'],
            kernel_size=params['kernel_size'],
        ...
        up_shape = [2 * int(common_flow.shape[i]) for i in (1, 2, 3)]
        upsample_op = LinearResizeLayer(up_shape)
        common_flow = upsample_op(common_flow)
